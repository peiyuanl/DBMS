...
public:
  ...
  int dataLength(const vector<Attribute> &recordDescriptor, const void *data);

  void NewPage(FileHandle &fileHandle);

  int NewIndexPage(FileHandle &fileHandle);

  int findPage(FileHandle &fileHandle, int length);

  int updatePage(FileHandle &fileHandle, int length, const void *data, int pageNum);
